...
function Player:loadAssets()
    ...
    for i=1, self.animation.run.total do
        self.animation.run.img[i] = love.graphics.newImage("assets/player/Swordsman/run/"..i..".png")
    end

    self.animation.idle = {total = 8, current = 1, img = {}}
    for i=1, self.animation.idle.total do
        self.animation.idle.img[i] = love.graphics.newImage("assets/player/Swordsman/idle/"..i..".png")
    end

    self.animation.jump = {total = 8, current = 1, img = {}}
    for i=1, self.animation.jump.total do
        self.animation.jump.img[i] = love.graphics.newImage("assets/player/Swordsman/jump/"..i..".png")
    end
...
function Player:jump(key)
    if (key == "w" or key == "up") and (self.grounded or self.doubleJump <= 1) then
        if not self.grounded and self.doubleJump == 0 then self.doubleJump = 1
        end
